use crate::wf_convert::MessageCodeParser;
...
fn test_t_message() {
    ...
    let message: BasicMessage = crate::wf_convert::compile(&field_values);
    //let encoded_message = super::creator::encode(&field_values);
    //let decoded_message = super::creator::decode(&encoded_message);

    //assert_eq!(message_serialized, &encoded_message, "message should encode correctly");
    println!("{:#?}", MessageCodeParser::parse(&field_values));
    assert_eq!(
        message_serialized,
        &message.serialize(),
