...
use bp256::r1::{
    AffinePoint as BrainpoolAffine, BrainpoolP256r1, NonZeroScalar as BrainpoolNonZeroScalar,
    ProjectivePoint as BrainpoolProjective, SecretKey as BrainpoolSecretKey,
};
...
impl WhiteflagECDHKeyPair {
    ...
    pub fn from_bytes(secret_bytes: &[u8; 32]) -> Self {
        let secret_key =
            BrainpoolSecretKey::from_bytes(secret_bytes.into()).expect("Invalid secret key bytes");
        let secret_scalar = secret_key.to_nonzero_scalar();
    ...
    pub fn create_aes_cipher(&self, peer_public_bytes: &[u8]) -> AESCipher {
        let aes_key = self
            .derive_aes256_key(peer_public_bytes)
