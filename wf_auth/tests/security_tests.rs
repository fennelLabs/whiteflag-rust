...
mod security_tests {
    ...
    fn test_token_tampering_detection() {
        ...
        let token = jwt_auth
            .create_method1_token("https://test.example.com/auth", None)
            .unwrap();
    ...
    fn test_expired_token_detection() {
        ...
        let token = jwt_auth
            .create_method1_token("https://test.example.com/auth", Some(claims))
            .unwrap();
    ...
    fn test_key_derivation_consistency() {
        ...
        assert_eq!(
            ecdsa1.to_encoded_point(false),
            ecdsa2.to_encoded_point(false)
        );
    }
    ...
    fn test_signature_uniqueness() {
        ...
        let header1: serde_json::Value =
            serde_json::from_slice(&general_purpose::URL_SAFE_NO_PAD.decode(parts1[0]).unwrap())
                .unwrap();
        let header2: serde_json::Value =
            serde_json::from_slice(&general_purpose::URL_SAFE_NO_PAD.decode(parts2[0]).unwrap())
                .unwrap();
    ...
    fn test_large_claims_handling() {
        ...
        for i in 0..100 {
            large_claims.insert(
                ...
                serde_json::json!(format!(
                    "value_{}_with_some_longer_content_to_test_size_handling",
                    i
                )),
            );
        }
        ...
        let token = jwt_auth
            .create_method1_token("https://test.example.com/auth", Some(large_claims))
            .unwrap();
    ...
    fn test_special_characters_in_claims() {
        ...
        let token = jwt_auth
            .create_method1_token("https://test.example.com/auth", Some(special_claims))
            .unwrap();
    ...
    fn test_concurrent_token_generation() {
        ...
        for i in 0..10 {
            ...
            let handle = thread::spawn(move || {
                ...
                jwt_auth.create_method1_token("https://test.example.com/auth", None)
            });
            handles.push(handle);
        }
