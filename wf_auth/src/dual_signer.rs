...
use schnorrkel::{Keypair as Sr25519Keypair, PublicKey as Sr25519PublicKey, Signature as Sr25519Signature};
...
impl WhiteflagSigner {
    ...
    pub fn from_seed(seed: &[u8; 32]) -> Result<Self, SignerError> {
        ...
        let mini_secret = MiniSecretKey::from_bytes(seed)
            .map_err(|e| SignerError::KeyGeneration(format!("sr25519 mini secret from seed: {}", e)))?;
    ...
    pub fn sign_ecdsa(&self, data: &[u8]) -> Result<EcdsaSignature, SignerError> {
        ...
        let signature = self.ecdsa_key.try_sign(&hash)
    ...
    pub fn create_hybrid_auth(
        &self,
        data: &[u8],
    ) -> Result<HybridAuth, SignerError> {
...
mod tests {
    ...
    fn test_whiteflag_jws_creation() {
        ...
        let jws = signer.create_whiteflag_jws(
            "whiteflag-validator",
            "whiteflag-network",
            now,
            now + 3600,
