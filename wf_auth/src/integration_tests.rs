...
mod integration_tests {
    ...
    fn test_complete_whiteflag_jwt_workflow() {
        ...
        assert!(matches!(auth_token.as_ref(), &AuthenticationMethod::PresharedToken));
        ...
        assert!(claims.whiteflag_claims.get("method").unwrap().as_i64().unwrap() == 1);
        assert_eq!(
            claims.whiteflag_claims.get("resource").unwrap().as_str().unwrap(),
            "https://fennel.network/whiteflag/auth"
        );
    ...
    fn test_dual_algorithm_compatibility() {
        ...
        assert!(WhiteflagSigner::verify_ecdsa(
        ...
        println!("SR25519 signature length: {} bytes", sr25519_sig.to_bytes().len());
        println!("ECDSA signature length: {} bytes", ecdsa_sig.to_bytes().len());
    }
    ...
    fn test_extended_auth_token_functionality() {
        ...
        assert!(payload.whiteflag_claims.get("method").unwrap().as_i64().unwrap() == 1);
        assert_eq!(
            payload.whiteflag_claims.get("resource").unwrap().as_str().unwrap(),
            "https://api.fennel.network/auth"
        );
...
mod performance_tests {
    ...
    fn test_signing_performance() {
        ...
        println!("  SR25519: {:?} ({:.2} ms/op)", sr25519_duration, sr25519_duration.as_millis() as f64 / iterations as f64);
        println!("  ECDSA:   {:?} ({:.2} ms/op)", ecdsa_duration, ecdsa_duration.as_millis() as f64 / iterations as f64);
    ...
    fn test_jwt_creation_performance() {
        ...
        for i in 0..iterations {
            let _token = jwt_auth.create_method1_token(
                &format!("https://example.com/auth/{}", i),
                None,
            ).unwrap();
        }
        
        let duration = start.elapsed();
        println!("📊 JWT Creation Performance ({} iterations): {:?} ({:.2} ms/op)", 
                iterations, duration, duration.as_millis() as f64 / iterations as f64);
