...
impl WhiteflagApiClient {
    ...
    async fn test_connectivity(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if response.status().is_success() {
            ...
            match serde_json::from_str::<Value>(&text) {
                ...
                Err(_) => {
                    Err(format!("Response is not valid JSON - not a Whiteflag API").into())
                }
            }
        } else {
    ...
    async fn get_blockchains(&self) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        if response.status().is_success() {
            ...
            if let Some(data) = json.get("data").and_then(|d| d.as_array()) {
                let blockchains: Vec<String> = data.iter()
    ...
    async fn verify_signature(&self, signature_data: Value) -> Result<Value, Box<dyn std::error::Error>> {
        ...
        if response.status().is_success() {
            ...
            println!("✅ Signature verification response: {}", serde_json::to_string_pretty(&result)?);
            Ok(result)
        } else {
    ...
    async fn decode_signature(&self, signature_data: Value) -> Result<Value, Box<dyn std::error::Error>> {
        ...
        if response.status().is_success() {
            let result: Value = response.json().await?;
            println!("✅ Signature decode response: {}", serde_json::to_string_pretty(&result)?);
            Ok(result)
        } else {
            let status = response.status();
            let error_text = response.text().await?;
    ...
    async fn send_message(&self, message: Value) -> Result<Value, Box<dyn std::error::Error>> {
        ...
        if response.status().is_success() {
            let result: Value = response.json().await?;
            println!("✅ Message send response: {}", serde_json::to_string_pretty(&result)?);
            Ok(result)
        } else {
            let status = response.status();
            let error_text = response.text().await?;
...
fn create_whiteflag_auth_payload(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth_payload = create_whiteflag_auth_payload(
        blockchain_address,
        &originator_pubkey,
    );
...
async fn demonstrate_jwt_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let demo_message = create_auth1_message(
        "0x742d35Cc6634C0532925a3b8D2Cc1A51e37DcFF1",
        "1",
        &jwt,
    );
