...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📋 SR25519 Public Key: {}",
        hex::encode(sr25519_pk.to_bytes())
    );
    println!(
        "   📋 ECDSA Public Key: {}",
        hex::encode(ecdsa_pk.to_encoded_point(false))
    );
    ...
    println!(
        "   ✅ JWT auth configured for subject: {}",
        jwt_auth.subject()
    );
    println!(
        "   ✅ JWT auth configured for audience: {}",
        jwt_auth.audience()
    );
    ...
    println!(
        "   📋 Issued at: {}",
        chrono::DateTime::from_timestamp(claims.iat, 0).unwrap()
    );
    println!(
        "   📋 Expires at: {}",
    ...
    println!(
        "   📋 Whiteflag method: {}",
        claims.whiteflag_claims.get("method").unwrap()
    );
    println!(
        "   📋 Internet resource: {}",
        claims.whiteflag_claims.get("resource").unwrap()
    );
    ...
    println!(
        "   📋 ECDSA sig length: {} bytes",
        ecdsa_sig.to_bytes().len()
    );

    // Verify both signatures
    let sr25519_valid =
        WhiteflagSigner::verify_sr25519(&signer.sr25519_public_key(), &sr25519_sig, test_message);
    let ecdsa_valid =
        WhiteflagSigner::verify_ecdsa(&signer.ecdsa_public_key(), &ecdsa_sig, test_message)?;
    ...
    println!(
        "   ✅ Hybrid authentication created and verified: {}",
        hybrid_valid
    );

    let compact_hybrid = hybrid_auth.to_compact()?;
    println!(
        "   📋 Compact hybrid auth length: {} characters",
        compact_hybrid.len()
    );
    ...
    println!(
        "   📋 Deterministic SR25519 PK: {}",
        hex::encode(deterministic_signer.sr25519_public_key().to_bytes())
    );
    ...
    println!(
        "   ✅ Generated verification data: {} bytes",
